...
const Post = ({ userAvatar, userName, bookImg, bookTitle, bookDescription, bookLink, timeStamp }) => {
  ...
  const dateObject = new Date(date[2], date[1] - 1, date[0], time[0], time[1], time[2]);
    ...
    <Card className='post-container'>
      ...
      <CardContent className='post-content'>
        ...
        <div className='post-content-right'>
          <a href={bookLink} target='_blank' rel='noopener noreferrer' className='book-link'>
            <Typography variant='h6' className='book-title'>
              {bookTitle}
            </Typography>
          </a>
          <Select value={status} onChange={handleChange} variant='outlined' size='small' className='status-select'>
          ...
          <div className='rating-section'>
            ...
            {Array.from({ length: 5 }, (_, index) => (
              <IconButton key={index} aria-label='rate' onClick={() => setRating(index + 1)}>
                {rating > index ? <StarIcon style={{ color: "gold" }} /> : <StarBorderIcon />}
              </IconButton>
            ))}
          </div>
      ...
      <CardActions className='post-actions'>
        <IconButton onClick={toggleLike}>{liked ? <FavoriteIcon color='error' /> : <FavoriteBorderIcon />}</IconButton>
      ...
      {showComment && (
        <CardContent className='comment-section'>
          <div className='comment-input-container'>
            <Avatar src={userAvatar} alt='User Avatar' className='comment-avatar' />
            <TextField
              variant='outlined'
              size='small'
              ...
              fullWidth
            />
            <IconButton onClick={handleCommentSubmit} color='primary' aria-label='send comment'>
              <SendIcon />
            </IconButton>
          </div>
        </CardContent>
